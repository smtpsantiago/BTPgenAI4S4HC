...
/**
 * 
 * @After(event = { "CREATE","UPDATE" }, entity = "josebecerra_07_a19Srv.ProductFAQ")
...
module.exports = async function(results, request) {
	...
	try {
		...
		let productFAQ = await SELECT.one('josebecerra_07_a19.ProductFAQ').where({ ID: productFAQID }).forUpdate();
		...
		await UPDATE("josebecerra_07_a19.ProductFAQ", ID).with({ embedding: utils.array2VectorBuffer(embedding) });
