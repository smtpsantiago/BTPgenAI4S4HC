...
/**
 * 
 * @On(event = { "Action1" }, entity = "josebecerra_07_a19Srv.CustomerMessage")
...
module.exports = async function(request) {
    ...
    try {
        ...
        const customerMessage = await SELECT.one.from('josebecerra_07_a19.CustomerMessage').where({ ID });
        ...
        if (messageCategory === 'Technical') {
            ...
            const relevantFAQs = await SELECT`id, issue, question, answer`
                .from('josebecerra_07_a19.ProductFAQ')
        ...
        await UPDATE('josebecerra_07_a19.CustomerMessage').set({
            suggestedResponseCustomerLanguage,
            suggestedResponseEnglish,
        }).where({ ID });
